...
SimulatorHeartbeat::SimulatorHeartbeat( Config* UNUSED(cfg), int this_rank, Simulation_impl* sim, TimeConverter* period) :
    ...
    m_period( period )
{
    // printf("Test 0 in SimulatorHeartbeat of heartbeat.cc\n");
    fflush(stdout);
    sim->insertActivity( period->getFactor(), this );
    if( (0 == this_rank) ) {
        lastTime = sst_get_cpu_time();
    }
    // printf("Test 1 in SimulatorHeartbeat of heartbeat.cc\n");
    fflush(stdout);
    // if( (0 == this_rank) ) {
    //     sim->insertActivity( period->getFactor(), this );
    //     lastTime = sst_get_cpu_time();
...
{
    // printf("Test 0 in SimulatorHeartbeat::execute of heartbeat.cc\n");
    fflush(stdout);
    ...
    // printf("Test 1 in SimulatorHeartbeat::execute of heartbeat.cc\n");
    fflush(stdout);
