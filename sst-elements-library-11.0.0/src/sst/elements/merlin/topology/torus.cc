...
{
    // printf("Test 0 in route_packet of torus.cc %d\n", router_id);
    int dest_router = get_dest_router(ev->getDest());
    if ( dest_router == router_id ) {
        // printf("Test 01 eject in route_packet of torus.cc\n");
        ...
        for ( int dim = tt_ev->routing_dim ; dim < dimensions ; dim++ ) {
        	...
        	// printf("Test 02 next port %d in route_packet of torus.cc\n", p);
                tt_ev->setNextPort(p);
    ...
    // printf("Test 1 in route_packet of torus.cc\n");
}
