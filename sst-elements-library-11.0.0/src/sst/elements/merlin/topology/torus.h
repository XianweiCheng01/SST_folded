...
public:
	...
	// printf("Test 0 in topo_torus_event::serialize_order of torus.h\n");
	fflush(stdout);
        internal_router_event::serialize_order(ser);
	// printf("Test 1 in topo_torus_event::serialize_order of torus.h\n");
	fflush(stdout);
        ser & dimensions;
        ser & routing_dim;
	// printf("Test 2 in topo_torus_event::serialize_order of torus.h\n");
	fflush(stdout);

        if ( ser.mode() == SST::Core::Serialization::serializer::UNPACK ) {
            dest_loc = new int[dimensions];
        }
	// printf("Test 3 in serialize_order of torus.h\n");
	fflush(stdout);

        for ( int i = 0 ; i < dimensions ; i++ ) {
            ser & dest_loc[i];
        }
	// printf("Test 4 in serialize_order of torus.h\n");
	fflush(stdout);
    }
