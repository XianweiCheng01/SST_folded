...
{
    // printf("Test 0 of hr_router in hr_router.cc\n");
    fflush(stdout);
    ...
    init_vcs();
    // printf("Test 1 of hr_router in hr_router.cc\n");
    fflush(stdout);
}
...
hr_router::clock_handler(Cycle_t cycle)
{
    ...
    for ( int i = 0; i < num_ports; i++ ) {
	    ...
	    // printf("Test 0 in clock handler of hr_router.cc router %d send event (src: %d, dest: %d) from port %d\n", id, ev->getSrc(), ev->getDest(), ev->getNextPort());
...
{
    // printf("Test 0 in hr_router.cc %d\n", num_ports);
    fflush(stdout);
    for ( int i = 0; i < num_ports; i++ ) {
        ports[i]->init(phase);
        Event *ev = NULL;
        while ( (ev = ports[i]->recvInitData()) != NULL ) {
            ...
            for ( std::vector<int>::iterator j = outPorts.begin() ; j != outPorts.end() ; ++j ) {
                ...
                case Topology::R2N:
		    // printf("Test 0 1 in init of hr_router.cc");
		    fflush(stdout);
		    ...
		    // printf("Test 0 2 in init of hr_router.cc");
		    fflush(stdout);
    ...
    for ( int i = 0; i < num_ports; i++ ) {
        ...
        while ( (ev = ports[i]->recvInitData()) != NULL ) {
            ...
            for ( std::vector<int>::iterator j = outPorts.begin() ; j != outPorts.end() ; ++j ) {
                ...
                switch ( topo->getPortState(*j) ) {
                case Topology::R2N:
		    // printf("Test 0 1 in complete of hr_router.cc");
		    fflush(stdout);
                    ports[*j]->sendUntimedData(ire->getEncapsulatedEvent()->clone());
                    break;
                case Topology::R2R:
                // Ignore failed links during init
                case Topology::FAILED: {
		    // printf("Test 0 2 in complete of hr_router.cc");
		    fflush(stdout);
                    internal_router_event *new_ire = ire->clone();
                    new_ire->setEncapsulatedEvent(ire->getEncapsulatedEvent()->clone());
                    ports[*j]->sendUntimedData(new_ire);
