...
void *
graph_500_class::xmalloc (size_t sz)
{
  void * out;
  if (!numa_inited) {
    OMP("omp critical") {
...
void *
graph_500_class::xcalloc (size_t n, size_t sz)
{
  void * out;
  if (!numa_inited) {
    OMP("omp critical") {
...
void
graph_500_class::xfree (void * p, size_t sz)
{
  if (!p) return;
  if (numa_avail >= 0)
    numa_free (p, sz);
 ...
 * range. */
void graph_500_class::make_mrg_seed(uint64_t userseed1, uint64_t userseed2, uint_fast32_t* seed) {
