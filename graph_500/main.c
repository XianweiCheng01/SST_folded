...
*/
#include <math.h>
#include "graph_500_class.h"
//#include "graph_generator.h"
//#define sstmac_app_name app_graph_bfs_sssp
...
void graph_500_class::get_statistics(const double x[], int n, volatile double r[s_LAST]) {
	double temp;
	int i;
	/* Compute mean. */
	temp = 0.0;
...
int main(int argc, char** argv) {
	printf("Test 0 in main.c\n");
	fflush(stdout);
	int r;
	printf("-------------------------Test 1 in main.c------------------------\n");
	fflush(stdout);
	r = MPI_Init(&argc, &argv);
	printf("-------------------------Test 2 in main.c------------------------\n");
	fflush(stdout);
	if ( r != MPI_SUCCESS ) return r;

	graph_500_class *my_graph_500;
	my_graph_500 = new graph_500_class();

	printf("Test 3 in main.c\n");
	fflush(stdout);
	my_graph_500->aml_init(&argc,&argv); //includes MPI_Init inside
	printf("Test 4 in main.c\n");
	fflush(stdout);
	my_graph_500->setup_globals();
	printf("Test 5 in main.c\n");
	fflush(stdout);
	...
	// const bool generate_data = getenv("GENERATE")?true:false;
	...
	char *wfilename=NULL;  
	if(filename!=NULL) {
		wfilename=(char *)malloc(strlen(filename)+8);
...
#if 0
	if (tg.data_in_file) {
		printf("Data in file is running...\n");
		int is_opened = 0;
	 ...
	 * validator). */
	int num_bfs_roots = 1;
	int64_t* bfs_roots = (int64_t*)my_graph_500->xmalloc(num_bfs_roots * sizeof(int64_t));

	double make_graph_start = MPI_Wtime();

	char format_edge[] = "edge-%d-%d.txt";
	char format_weight[] = "weight-%d-%d.txt";
	char edge_filename[sizeof(format_edge)+10];
	char weight_filename[sizeof(format_weight)+10];
	sprintf(edge_filename, format_edge, SCALE, edgefactor);
	sprintf(weight_filename, format_weight, SCALE, edgefactor);
	FILE * fp_edge;
	FILE * fp_weight;

	//my_graph_500->aml_finalize(); //includes MPI_Finalize()
	//return 0;
	/*if (0) {
//printf("Test 0 in main.c\n");
fflush(stdout);
		tg.edgememory_size = tg.nglobaledges;
		tg.edgememory = (packed_edge*)my_graph_500->xmalloc(tg.nglobaledges * sizeof(packed_edge));
		tg.weightmemory = (float*)my_graph_500->xmalloc(tg.nglobaledges*sizeof(float));

		fp_edge = fopen(edge_filename, "r");
		fp_weight = fopen(weight_filename, "r");
		fread(tg.edgememory, sizeof(packed_edge), tg.nglobaledges, fp_edge);
		fread(tg.weightmemory, sizeof(float), tg.nglobaledges, fp_weight);
...
fflush(stdout);
	...
	if( !tg.data_in_file || tg.write_file )
	{
		...
		my_graph_500->make_mrg_seed(seed1, seed2, seed);
		 ...
		 * roots. */
		MPI_Offset nchunks_in_file = (tg.nglobaledges + my_graph_500->FILE_CHUNKSIZE - 1) / my_graph_500->FILE_CHUNKSIZE;
		int64_t bitmap_size_in_bytes = my_graph_500->int64_min(my_graph_500->BITMAPSIZE, (nglobalverts + CHAR_BIT - 1) / CHAR_BIT);
		if (bitmap_size_in_bytes * my_graph_500->size * CHAR_BIT < nglobalverts) {
			bitmap_size_in_bytes = (nglobalverts + my_graph_500->size * CHAR_BIT - 1) / (my_graph_500->size * CHAR_BIT);
		}
		int ranks_per_row = ((nglobalverts + CHAR_BIT - 1) / CHAR_BIT + bitmap_size_in_bytes - 1) / bitmap_size_in_bytes;
		int nrows = my_graph_500->size / ranks_per_row;
		int my_row = -1, my_col = -1;
		MPI_Comm cart_comm;
		{
			int dims[2] = {my_graph_500->size / ranks_per_row, ranks_per_row};
			int periods[2] = {0, 0};
		...
		if (cart_comm != MPI_COMM_NULL) {
			...
			packed_edge* buf = (packed_edge*)my_graph_500->xmalloc(my_graph_500->FILE_CHUNKSIZE * sizeof(packed_edge));
#ifdef SSSP
			float* wbuf = (float*)my_graph_500->xmalloc(my_graph_500->FILE_CHUNKSIZE*sizeof(float));
#endif
			...
			if (tg.data_in_file) {
				...
				int last_pos = (tg.nglobaledges % ((int64_t)my_graph_500->FILE_CHUNKSIZE * nrows * ranks_per_row) != 0) ?
					(tg.nglobaledges / my_graph_500->FILE_CHUNKSIZE) % (nrows * ranks_per_row) :
					-1;
				int64_t edges_left = tg.nglobaledges % my_graph_500->FILE_CHUNKSIZE;
				int64_t nedges = my_graph_500->FILE_CHUNKSIZE * (tg.nglobaledges / ((int64_t)my_graph_500->FILE_CHUNKSIZE * nrows * ranks_per_row)) +
					my_graph_500->FILE_CHUNKSIZE * (my_pos < (tg.nglobaledges / my_graph_500->FILE_CHUNKSIZE) % (nrows * ranks_per_row)) +
				...
				tg.edgememory = (packed_edge*)my_graph_500->xmalloc(nedges * sizeof(packed_edge));
#ifdef SSSP
				tg.weightmemory = (float*)my_graph_500->xmalloc(nedges*sizeof(float));
#endif
			}
			...
			for (block_idx = 0; block_idx < block_limit; ++block_idx) {
				...
				MPI_Offset start_edge_index = my_graph_500->int64_min(my_graph_500->FILE_CHUNKSIZE * (block_idx * nrows + my_row), tg.nglobaledges);
				MPI_Offset edge_count = my_graph_500->int64_min(tg.nglobaledges - start_edge_index, my_graph_500->FILE_CHUNKSIZE);
				packed_edge* actual_buf = (!tg.data_in_file && block_idx % ranks_per_row == my_col) ?
					tg.edgememory + my_graph_500->FILE_CHUNKSIZE * (block_idx / ranks_per_row) :
					buf;
#ifdef SSSP
				float* actual_wbuf = (!tg.data_in_file && block_idx % ranks_per_row == my_col) ?
					tg.weightmemory + my_graph_500->FILE_CHUNKSIZE * (block_idx / ranks_per_row) :
					wbuf;
#endif
				fprintf(stderr, "%d: My range is [%" PRId64 ", %" PRId64 ") %swriting into index %" PRId64 "\n", rank, (int64_t)start_edge_index, (int64_t)(start_edge_index + edge_count), (my_col == (block_idx % ranks_per_row)) ? "" : "not ", (int64_t)(my_graph_500->FILE_CHUNKSIZE * (block_idx / ranks_per_row))); /**/
				if (!tg.data_in_file && block_idx % ranks_per_row == my_col) {
					assert (my_graph_500->FILE_CHUNKSIZE * (block_idx / ranks_per_row) + edge_count <= tg.edgememory_size);
				}
				if (tg.write_file) {
					if (generate_data) {
						my_graph_500->generate_kronecker_range(seed, SCALE, start_edge_index, start_edge_index + edge_count, actual_buf
#ifdef SSSP
							,actual_wbuf
#endif
							);
						...
						fseek(fp_edge, start_edge_index*sizeof(packed_edge), SEEK_SET);
						fseek(fp_weight, start_edge_index*sizeof(float), SEEK_SET);

						fread(actual_buf, sizeof(packed_edge), edge_count, fp_edge);
						fread(actual_wbuf, sizeof(float), edge_count, fp_weight);
		...
		if (generate_data) {
		// if (0) {
			fp_edge = fopen(edge_filename, "w+");
			fp_weight = fopen(weight_filename, "w+");

			fwrite(tg.edgememory, sizeof(packed_edge), tg.nglobaledges, fp_edge);
			fwrite(tg.weightmemory, sizeof(float), tg.nglobaledges, fp_weight);
...
/**/
	...
	if (my_graph_500->rank == 0) { /* Not an official part of the results */
		fprintf(stderr, "graph_generation:               %f s\n", make_graph_time);
	}

	/* Make user's graph data structure. */
	double data_struct_start = MPI_Wtime();
	my_graph_500->make_graph_data_structure(&tg);
	...
	#if 0
	{
		...
		for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
			...
			while (1) {
				double d[2];
				my_graph_500->make_random_numbers(2, seed1, seed2, counter, d);
				...
				int root_bad = my_graph_500->isisolated(root);
	 ...
	 * used directly. */
	double* edge_counts = (double*)my_graph_500->xmalloc(num_bfs_roots * sizeof(double));

	/* Run BFS. */
	int validation_passed = 1;
	double* bfs_times = (double*)my_graph_500->xmalloc(num_bfs_roots * sizeof(double));
	double* validate_times = (double*)my_graph_500->xmalloc(num_bfs_roots * sizeof(double));
	uint64_t nlocalverts = my_graph_500->get_nlocalverts_for_pred();
	int64_t* pred = (int64_t*)my_graph_500->xMPI_Alloc_mem(nlocalverts * sizeof(int64_t));
	float* shortest = (float*)my_graph_500->xMPI_Alloc_mem(nlocalverts * sizeof(float));

	int bfs_root_idx,i;
	if (!getenv("SKIP_BFS")) {
		my_graph_500->clean_pred(&pred[0]); //user-provided function from bfs_implementation.c
		my_graph_500->run_bfs(bfs_roots[0], &pred[0]); //warm-up
		if (!getenv("SKIP_VALIDATION")) {
			int64_t nedges=0;
			my_graph_500->validate_result(1,&tg, nlocalverts, bfs_roots[0], pred,shortest,NULL);
		}

		for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
			int64_t root = bfs_roots[bfs_root_idx];

			if (my_graph_500->rank == 0) fprintf(stderr, "Running BFS %d\n", bfs_root_idx);

			my_graph_500->clean_pred(&pred[0]); //user-provided function from bfs_implementation.c
			/* Do the actual BFS. */
			double bfs_start = MPI_Wtime();
			my_graph_500->run_bfs(root, &pred[0]);
			double bfs_stop = MPI_Wtime();
			bfs_times[bfs_root_idx] = bfs_stop - bfs_start;
			if (my_graph_500->rank == 0) fprintf(stderr, "Time for BFS %d is %f\n", bfs_root_idx, bfs_times[bfs_root_idx]);
			int64_t edge_visit_count=0;
			my_graph_500->get_edge_count_for_teps(&edge_visit_count);
			edge_counts[bfs_root_idx] = (double)edge_visit_count;
			if (my_graph_500->rank == 0) fprintf(stderr, "TEPS for BFS %d is %g\n", bfs_root_idx, edge_visit_count / bfs_times[bfs_root_idx]);

			/* Validate result. */
			if (!getenv("SKIP_VALIDATION")) {
				if (my_graph_500->rank == 0) fprintf(stderr, "Validating BFS %d\n", bfs_root_idx);

				double validate_start = MPI_Wtime();
				int validation_passed_one = my_graph_500->validate_result(1,&tg, nlocalverts, root, pred,shortest,&edge_visit_count);
				double validate_stop = MPI_Wtime();

				validate_times[bfs_root_idx] = validate_stop - validate_start;
				if (my_graph_500->rank == 0) fprintf(stderr, "Validate time for BFS %d is %f\n", bfs_root_idx, validate_times[bfs_root_idx]);

				if (!validation_passed_one) {
					validation_passed = 0;
					if (my_graph_500->rank == 0) fprintf(stderr, "Validation failed for this BFS root; skipping rest.\n");
					break;
				}
			} else
...
#ifdef SSSP
	double* sssp_times = (double*)my_graph_500->xmalloc(num_bfs_roots * sizeof(double));
	double* validate_times2 = (double*)my_graph_500->xmalloc(num_bfs_roots * sizeof(double));

	my_graph_500->clean_shortest(shortest);
	my_graph_500->clean_pred(pred);
	my_graph_500->run_sssp(bfs_roots[0], &pred[0],shortest); //warm-up

	for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
		int64_t root = bfs_roots[bfs_root_idx];

		if (my_graph_500->rank == 0) fprintf(stderr, "Running SSSP %d\n", bfs_root_idx);

		my_graph_500->clean_pred(&pred[0]);
		my_graph_500->clean_shortest(shortest);

		/* Do the actual SSSP. */
		double sssp_start = MPI_Wtime();
		my_graph_500->run_sssp(root, &pred[0],shortest);
		...
		if (my_graph_500->rank == 0) fprintf(stderr, "Time for SSSP %d is %f\n", bfs_root_idx, sssp_times[bfs_root_idx]);
		if (my_graph_500->rank == 0) fprintf(stderr, "TEPS for SSSP %d is %g\n", bfs_root_idx, edge_counts[bfs_root_idx] / sssp_times[bfs_root_idx]);

		/* Validate result. */
		if (!getenv("SKIP_VALIDATION")) {
			if (my_graph_500->rank == 0) fprintf(stderr, "Validating SSSP %d\n", bfs_root_idx);

			double validate_start = MPI_Wtime();
			int validation_passed_one = my_graph_500->validate_result(0,&tg, nlocalverts, root, pred, shortest,&edge_visit_count);
			double validate_stop = MPI_Wtime();

			validate_times2[bfs_root_idx] = validate_stop - validate_start;
			if (my_graph_500->rank == 0) fprintf(stderr, "Validate time for SSSP %d is %f\n", bfs_root_idx, validate_times2[bfs_root_idx]);

			if (!validation_passed_one) {
				validation_passed = 0;
				if (my_graph_500->rank == 0) fprintf(stderr, "Validation failed for this SSSP root; skipping rest.\n");
				break;
			}
		} else {
...
#endif
	free(bfs_roots);
	my_graph_500->free_graph_data_structure();

	if (tg.data_in_file) {
		//printf("File close is running...\n");
		//MPI_File_close(&tg.edgefile);
#ifdef SSSP
	...
	if (my_graph_500->rank == 0) {
		if (!validation_passed) {
			...
			volatile double stats[my_graph_500->s_LAST];
			my_graph_500->get_statistics(bfs_times, num_bfs_roots, stats);
			fprintf(stdout, "bfs  min_time:                  %g\n", stats[my_graph_500->s_minimum]);
			fprintf(stdout, "bfs  firstquartile_time:        %g\n", stats[my_graph_500->s_firstquartile]);
			fprintf(stdout, "bfs  median_time:               %g\n", stats[my_graph_500->s_median]);
			fprintf(stdout, "bfs  thirdquartile_time:        %g\n", stats[my_graph_500->s_thirdquartile]);
			fprintf(stdout, "bfs  max_time:                  %g\n", stats[my_graph_500->s_maximum]);
			fprintf(stdout, "bfs  mean_time:                 %g\n", stats[my_graph_500->s_mean]);
			fprintf(stdout, "bfs  stddev_time:               %g\n", stats[my_graph_500->s_std]);
#ifdef SSSP
			my_graph_500->get_statistics(sssp_times, num_bfs_roots, stats);
...
#endif
			my_graph_500->get_statistics(edge_counts, num_bfs_roots, stats);
			...
			double* secs_per_edge = (double*)my_graph_500->xmalloc(num_bfs_roots * sizeof(double));
			for (i = 0; i < num_bfs_roots; ++i) secs_per_edge[i] = bfs_times[i] / edge_counts[i];
			my_graph_500->get_statistics(secs_per_edge, num_bfs_roots, stats);
			fprintf(stdout, "bfs  min_TEPS:                  %g\n", 1. / stats[my_graph_500->s_maximum]);
			fprintf(stdout, "bfs  firstquartile_TEPS:        %g\n", 1. / stats[my_graph_500->s_thirdquartile]);
			fprintf(stdout, "bfs  median_TEPS:               %g\n", 1. / stats[my_graph_500->s_median]);
			fprintf(stdout, "bfs  thirdquartile_TEPS:        %g\n", 1. / stats[my_graph_500->s_firstquartile]);
			fprintf(stdout, "bfs  max_TEPS:                  %g\n", 1. / stats[my_graph_500->s_minimum]);
			fprintf(stdout, "bfs  harmonic_mean_TEPS:     !  %g\n", 1. / stats[my_graph_500->s_mean]);
			/* Formula from:
			...
			fprintf(stdout, "bfs  harmonic_stddev_TEPS:      %g\n", stats[my_graph_500->s_std] / (stats[my_graph_500->s_mean] * stats[my_graph_500->s_mean] * sqrt(num_bfs_roots - 1)));
#ifdef SSSP
			...
			my_graph_500->get_statistics(validate_times, num_bfs_roots, stats);
...
#ifdef SSSP
			my_graph_500->get_statistics(validate_times2, num_bfs_roots, stats);
...
int main(int argc, char* argv[])
{
    ...
    int r;
    r = MPI_Init(&argc, &argv);
    //printf("1 Get processor name is running...\n");
    fflush(stdout);
    if ( r != MPI_SUCCESS ) return r;

    graph_500_class *my_graph_500;
    my_graph_500 = new graph_500_class();
    return my_graph_500->ori_main(argc, argv);
}
*/
