...
int64_t maxvertex;
*/
void graph_500_class::frompredhndl(int from,void* data,int sz) { 
	int vfrom = *(int*)data;
...
void graph_500_class::send_frompred (int vfrom,int64_t src) {
	int pe=VERTEX_OWNER(src);
	int vloc[2]={vfrom, VERTEX_LOCAL(src)};
	aml_send(&vloc,1,8,pe);
}

void graph_500_class::vhalfedgehndl(int from,void* data,int sz)
{  vdegrees[*(int*)data]++; }

void graph_500_class::send_half (int64_t src) {
	int pe=VERTEX_OWNER(src);
	int vloc=VERTEX_LOCAL(src);
	aml_send(&vloc,1,4,pe);
}

void graph_500_class::vfulledgehndl(int frompe,void* data,int sz) {
	int vloc = *(int*)data;
...
void graph_500_class::vsend_full_edge (int64_t src,int64_t tgt,float w) {
	int pe=VERTEX_OWNER(src);
	int vloc[4];
...
*/
void graph_500_class::sendedgepreddist(unsigned vloc,unsigned int vedge) {
...
void graph_500_class::edgepreddisthndl(int frompe,void* data,int sz) {
...
void graph_500_class::makedepthmapforbfs(const size_t nlocalverts,const int64_t root,int64_t * const pred,float* dist) {
	...
	newvisits=1;
	prevlevel=0.0;
	aml_register_handler(&graph_500_class::frompredhndl,1);
...
int graph_500_class::validate_result(int isbfs,const tuple_graph* const tg, const size_t nlocalverts, const int64_t root, int64_t* const pred, float* dist,int64_t *nedges_in) {
	validatingbfs=isbfs;
...
vweights=weights;
	...
	aml_register_handler(&graph_500_class::edgepreddisthndl,1);
	nedges_traversed=0;
