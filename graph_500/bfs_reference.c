...
void graph_500_class::visithndl(int from,void* data,int sz) {
...
inline void graph_500_class::send_visit(int64_t glob, int from) {
	visitmsg m = {VERTEX_LOCAL(glob),from};
	aml_send(&m,1,sizeof(visitmsg),VERTEX_OWNER(glob));
}

void graph_500_class::make_graph_data_structure(const tuple_graph* const tg) {
	//printf("0 Test in call make graph\n");
	fflush(stdout);
	int i,j,k;
	//printf("1 Test in call make graph\n");
	fflush(stdout);
	convert_graph_to_oned_csr(tg, &g);
	//printf("2 Test in call make graph\n");
	fflush(stdout);
	column=g.column;
	rowstarts=g.rowstarts;
	//printf("3 Test in call make graph\n");
	fflush(stdout);

	visited_size = (g.nlocalverts + ulong_bits - 1) / ulong_bits;
	//printf("4 Test in call make graph\n");
	fflush(stdout);
	aml_register_handler(&graph_500_class::visithndl,1);
	//printf("5 Test in call make graph\n");
	fflush(stdout);
	q1 = (int *)xmalloc(g.nlocalverts*sizeof(int)); //100% of vertexes
	q2 = (int *)xmalloc(g.nlocalverts*sizeof(int));
	//printf("6 Test in call make graph\n");
	fflush(stdout);
	for(i=0;i<g.nlocalverts;i++) q1[i]=0,q2[i]=0; //touch memory
	visited = (long unsigned int*)xmalloc(visited_size*sizeof(unsigned long));
	//printf("7 Test in call make graph\n");
	fflush(stdout);
}

void graph_500_class::run_bfs(int64_t root, int64_t* pred) {
	int64_t nvisited;
	long sum;
...
void graph_500_class::get_edge_count_for_teps(int64_t* edge_visit_count) {
	long i,j;
	long edge_count=0;
...
void graph_500_class::clean_pred(int64_t* pred) {
	int i;
	for(i=0;i<g.nlocalverts;i++) pred[i]=-1;
}
void graph_500_class::free_graph_data_structure(void) {
	int i; 
	free_oned_csr_graph(&g);
	free(q1); free(q2); free(visited);
}

size_t graph_500_class::get_nlocalverts_for_pred(void) {
