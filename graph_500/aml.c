...
*/
SOATTR void graph_500_class::aml_register_handler(void(graph_500_class::*f)(int,void*,int),int n) {
	aml_barrier();
	aml_handlers[n]=f;
	aml_barrier();
}
...
void graph_500_class::process(int fromgroup,int length ,char* message) {
	int i = 0;
	...
	while ( i < length ) {
		...
		if(destlocal == mylocal)
			(this->*aml_handlers[hndl])(from,m+sizeof(struct hdr),hsz);
		else
...
void graph_500_class::process_intra(int fromlocal,int length ,char* message) {
	int i=0;
	while ( i < length ) {
		void*m = message+i;
		struct hdri *h = (hdri *)m;
		int hsz=h->sz;
		int hndl=h->hndl;
		(this->*aml_handlers[hndl])(PROC_FROM_GROUPLOCAL((int)(h->routing),fromlocal),m+sizeof(struct hdri),hsz);
		i += sizeof(struct hdri) + hsz;
	}
}

// poll intranode message
inline void graph_500_class::aml_poll_intra(void) {
	...
	if ( flag ) {
		...
		MPI_Recv_init( recvbuf_intra+AGGR_intra*index, AGGR_intra, MPI_CHAR,MPI_ANY_SOURCE, MPI_ANY_TAG, comm_intra,rqrecv_intra+index );
		MPI_Start( rqrecv_intra+index);
	}
}
// poll internode message
void graph_500_class::aml_poll(void) {
	int flag, from, length,index;
	MPI_Status status;
	...
	if ( flag ) {
		...
		MPI_Recv_init( recvbuf+AGGR*index, AGGR, MPI_CHAR,MPI_ANY_SOURCE, MPI_ANY_TAG, comm,rqrecv+index );
		MPI_Start( rqrecv+index );
	}
}

//flush internode buffer to destination node
inline void graph_500_class::flush_buffer( int node ) {
	MPI_Status stsend;
...
inline void graph_500_class::flush_buffer_intra( int node ) {
	MPI_Status stsend;
	int flag=0,index,tmp;
...
inline void graph_500_class::aml_send_intra(void *src, int type, int length, int local, int from) {
...
SOATTR void graph_500_class::aml_send(void *src, int type,int length, int node ) {
	if ( node == myproc )
		return (this->*aml_handlers[type])(myproc,src,length);
...
SOATTR int graph_500_class::aml_init( int *argc, char ***argv ) {
	int r, i, j,tmpmax;
	/*
	...
	*/
	...
	host_name[0] = 65;
	host_name[MPI_MAX_PROCESSOR_NAME-1] = '\0';
	...
	qsort(host_names, num_procs, sizeof(char[2]), stringCmp);
	color = 0;
	for (n=0; n<num_procs; n++)  {
	...
	pthread_setaffinity_np(pthread_self(), sizeof(sstmac_cpu_set_t), &cpuset);
#ifdef DEBUGSTATS
...
SOATTR void graph_500_class::aml_barrier( void ) {
	int i,flag;
	MPI_Request hndl;
	inbarrier++;
...
SOATTR void graph_500_class::aml_finalize( void ) {
	int i;
	aml_barrier();
	for(i=0;i<NRECV;i++) {
		//MPI_Cancel(rqrecv+i);
...
SOATTR int graph_500_class::aml_my_pe(void) { return myproc; }
SOATTR int graph_500_class::aml_n_pes(void) { return num_procs; }
