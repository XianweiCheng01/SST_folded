...
extern int64_t *pred_glob,visited_size;
...
#ifdef SSSP
void graph_500_class::relaxhndl(int from, void* dat, int sz) {
...
void graph_500_class::send_relax(int64_t glob, float weight,int fromloc) {
	relaxmsg m = {weight,VERTEX_LOCAL(glob),fromloc};
	aml_send(&m,1,sizeof(relaxmsg),VERTEX_OWNER(glob));
}

void graph_500_class::run_sssp(int64_t root,int64_t* pred,float *dist) {
	...
	aml_register_handler(&graph_500_class::relaxhndl,1);
...
void graph_500_class::clean_shortest(float* dist) {
	int i;
