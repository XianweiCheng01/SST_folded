...
int graph_500_class::isisolated(int64_t v) {
...
void graph_500_class::halfedgehndl(int from,void* data,int sz)
{  
// printf("Test 0 in halfedgehndl of csr_reference.c %d\n", *(int*)data);
// fflush(stdout);
degrees[*(int*)data]++;
}

void graph_500_class::fulledgehndl(int frompe,void* data,int sz) {
	int vloc = *(int*)data;
...
void graph_500_class::send_half_edge (int64_t src,int64_t tgt) {
	int pe=VERTEX_OWNER(src);
	int vloc=VERTEX_LOCAL(src);
// printf("Test 0 in send_half_edge of csr_reference.c %d, %d, %lld\n", vloc, pe, src);
// fflush(stdout);
	aml_send(&vloc,1,4,pe);
	if(tgt>=nverts_known) nverts_known=tgt+1;
}
#ifdef SSSP
void graph_500_class::send_full_edge (int64_t src,int64_t tgt,float w) {
	int pe=VERTEX_OWNER(src);
	int vloc[4];
...
#else
void graph_500_class::send_full_edge (int64_t src,int64_t tgt) {
	int pe=VERTEX_OWNER(src);
	int vloc[3];
	vloc[0]=VERTEX_LOCAL(src);
	memcpy(vloc+1,&tgt,8);
...
void graph_500_class::convert_graph_to_oned_csr(const tuple_graph* const tg, oned_csr_graph* const g) {
	g->tg = tg;
	...
	aml_register_handler(&graph_500_class::halfedgehndl,1);
	int numiters=ITERATE_TUPLE_GRAPH_BLOCK_COUNT(tg);
	// printf("Test 0 in convert_graph_to_oned_csr of csr_reference.c\n");
	fflush(stdout);
	// First pass : calculate degrees of each vertex
	ITERATE_TUPLE_GRAPH_BEGIN(tg, buf, bufsize,wbuf) {
		ptrdiff_t j;
	// printf("Test -1 in convert_graph_to_oned_csr of csr_reference.c %d\n", bufsize);
	fflush(stdout);
		for (j = 0; j < bufsize; ++j) {
	...
	// printf("Test 1 in convert_graph_to_oned_csr of csr_reference.c\n");
	fflush(stdout);
	...
	aml_register_handler(&graph_500_class::fulledgehndl,1);
...
void graph_500_class::free_oned_csr_graph(oned_csr_graph* const g) {
